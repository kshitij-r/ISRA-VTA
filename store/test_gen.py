...
for value in data:
    if (value == "instruction_dtype"):
        instr_dtype.append(data["instruction_dtype"])
instr_dtype = [item for sublist in instr_dtype for item in sublist]
# print(instr_dtype)
...
with open("store_autogen.cpp",'w',encoding = 'utf-8') as file:
    ...
    for x in range(len(instr_template)):
        file.write(instr_dtype[x] + " " + instr_template[x] + " = 0" + ";" + '\n')
    ...
    for count in range(len(instr_template)):
        if(count != len(instr_template)-1):
            file.write(instr_template[count] + ", ")
        else:
            file.write(instr_template[count])
    file.write("};\n")
    ...
    microop_entry_stage = []
    for value in data:
        if (value == "microinstruction_entry_stage"):
            microop_entry_stage.append(data["microinstruction_entry_stage"])
    microop_entry_stage = [item for sublist in microop_entry_stage for item in sublist]
    # print(microop_entry_stage)
    f = open('source.json',)
    source = json.load(f)
    # print(source)
    source_stages = []
    ...
    for key, value in source.items():
        source_stages.append(key)
        source_template.append(value)
    # print(source_stages)
    # print(source_template[0])
    current_stage = 0
    split_stage = 0
    for x in range(len(microop_entry_stage)):
        split_stage = microop_entry_stage[x]
        # print(split_stage)

    # split_stage = map(int, microop_entry_stage[0].split())
    # split_stage = int(microop_entry_stage)-0
    # print(split_stage)
    
    for x in range(microop_entry_stage[0]):
        source_flat = []
        file.write(source_stages[x] + "(" )
        for y in range(len(source_template[x])):
            if(y != len(source_template[x])-1):
                file.write(str(source_template[x][y]) + ", ")
            else:
                file.write(str(source_template[x][y]))
        file.write(");\n")
    with open("comment.txt",'r',encoding = 'utf-8' ) as file3:
        for line in file3:
            file.write(line)
    file.write('\n')
    for x in range(split_stage,len(source_stages)):
        # print(1)
        # source_flat = []
        file.write(source_stages[x] + "(" )
        for y in range(len(source_template[x])):
    ...
    for value in data:
        if (value == "assertion"):
            ...
            assertion_top.append(data["assertion"])
    assertion_top_flat = [item for sublist in assertion_top for item in sublist]
    # print(assertion_top)
    # print(assertion_top_flat)
    
    
    def assertion_statement(res):
        if ('not' in res):
            file.write("                assert(" + res[0] + "[k]!=" + res[2]+"[k]);\n")
        else:
            file.write("                assert(" + res[0] + "[k]==" + res[1]+"[k]);\n")
    for value in assertion_top_flat:
        res = [sub[value] for sub in assertion_top]
        res = [item for sublist in res for item in sublist]
        assertion_statement(res)
    file.write("k++;\n")
    file.write("}\n}\n}\n}")
    file.write("return 0;\n")
    file.write("}")
    # print(res)
    # for value in range(len(assertion_top)):
    #     print(value)
    # print(assertion_top)
