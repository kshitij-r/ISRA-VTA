          ...
          store_data_x14: for (int x14 = 0; x14 < ((unsigned int)x_size); ++x14) {
            ...
            mutate0_row: for (int row = 0; row < 1; ++row) {
              mutate0_col: for (int col = 0; col < 16; ++col) {
                DRAM[(((scalar2 * 16U) + ((unsigned int)(row * 16))) + ((unsigned int)col))] = out_mem[0U][0U][(((unsigned int)(col + (row * 16))) + (scalar1 * 16U))];
              }
            }
          }
        }
...
unsigned int insn_count;
 ...
 unsigned int acc_mem[131072][1][16];
 ap_uint<8> out_mem[32768][1][16];
 unsigned int uop_mem[32768];
 ap_uint<8> DRAM[134217728];
// klee_make_symbolic(&insn_addr, sizeof(insn_addr), "insn_addr");
//klee_make_symbolic(&insn_count, sizeof(insn_count), "insn_count");
/*
klee_make_symbolic(&inp_mem, sizeof(inp_mem), "inp_mem");
klee_make_symbolic(&wgt_mem, sizeof(wgt_mem), "wgt_mem");
klee_make_symbolic(&acc_mem, sizeof(acc_mem), "acc_mem");
klee_make_symbolic(&out_mem, sizeof(out_mem), "out_mem");
klee_make_symbolic(&uop_mem, sizeof(uop_mem), "uop_mem");*/

//insn_addr =  6.490371692147998911318997151514624e33;

//insn_count = 1;
//instr = 6.490371692147998911318997151514624e33;
insn_count =  7;
insn_addr= 0x2003000;

for (ap_int<32> i = 0; i < 32768; ++i) {
...
for (ap_int<32> i = 0; i < 134217728; ++i) {
                        DRAM[i] = 1;
...
default_function(insn_addr,insn_count, inp_mem, wgt_mem, acc_mem, out_mem, uop_mem, DRAM);

//default_function(instr, out_mem,DRAM_2);
